"""
롱 전용 vs 롱/숏 피라미딩 전략 비교 백테스트

사용법:
    python scripts/compare_ls_backtest.py --data data/candles/KRW_BTC_365d.parquet
    python scripts/compare_ls_backtest.py --data data/candles/KRW_BTC_365d.parquet \\
...
def run(df: pd.DataFrame, strategy, capital: float):
...
def main() -> None:
    parser = argparse.ArgumentParser(description="롱 vs 롱/숏 피라미딩 전략 비교")
    parser.add_argument("--data", type=Path, required=True, help="Parquet 데이터 파일 경로")
    parser.add_argument("--capital", type=float, default=1_000_000, help="초기 자본금 (기본: 1,000,000)")
    parser.add_argument("--params", nargs="*", default=[], metavar="KEY=VALUE",
                        help="전략 파라미터 (예: entry_pct=5.0 stop_pct=3.0 unit_amount=200000)")
    ...
    long_only = PyramidBreakoutStrategy(**params) if params else PyramidBreakoutStrategy()
    long_short = PyramidBreakoutLSStrategy(**params) if params else PyramidBreakoutLSStrategy()

    print(f"파라미터: {params or '기본값'}")
    print(f"자본금: {args.capital:,.0f}원\n")

    print("백테스트 실행 중...")
    result_long = run(df, long_only, args.capital)
    result_ls = run(df, long_short, args.capital)
