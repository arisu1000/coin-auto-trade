...
7. [매수 금지 및 수동 매도](#7-매수-금지-및-수동-매도)
8. [부분 익절 및 설정 핫 리로드](#8-부분-익절-및-설정-핫-리로드)
9. [로그 및 매매 내역 확인](#9-로그-및-매매-내역-확인)
10. [모의매매 → 실거래 전환](#10-모의매매--실거래-전환)
...
매 5분마다:
  - 포트폴리오 스냅샷 저장
  - 낙폭 감시 (매크로 킬 스위치)
  - 거래지원 종료 예정 마켓 감지 (신규 발견 시 자동 매도 + 텔레그램 알림)

KST 자정마다:
  - 일일 성과 리포트 텔레그램 자동 발송
    (30일 승률·손익·MDD, 현재 자산, 보유 포지션별 미실현 손익)
```
...
특정 종목만 골라서 시장가 매도합니다:

```
/sell           ← 보유 종목 목록을 버튼으로 표시
/sell KRW-BTC   ← 바로 확인 버튼으로 진행
```
...
킬스위치 상태는 **재시작 후에도 유지**됩니다 (SQLite 영속화).

### 킬스위치 해제

```
/resume           ← 전체 해제 (매크로·수동)
/resume KRW-BTC   ← KRW-BTC 마이크로 킬스위치만 해제
```
...
고점으로 판단되거나 거래하고 싶지 않은 마켓을 실시간으로 매수 금지할 수 있습니다:
...
`.env`의 `EXCLUDED_MARKETS=KRW-SHIB,KRW-FLOKI` 로 시작부터 정적으로 제외할 수도 있습니다.

### 거래지원 종료 예정 코인 자동 처리

업비트가 거래지원 종료 예정으로 지정한 마켓은 자동으로 처리됩니다:

- **보유 중인 경우**: 감지 즉시 시장가 자동 매도 + 텔레그램 알림
- **미보유인 경우**: 신규 매수 차단 + 텔레그램 알림

알림은 최초 감지 시 1회만 전송되며, 재시작 후 중복 알림이 발생하지 않습니다.

### 특정 종목 수동 매도

```
/sell           ← 보유 종목을 버튼으로 선택
/sell KRW-BTC   ← 직접 지정해서 확인
```
...
목표 수익률에 도달하면 보유량의 일부를 자동으로 매도하고, 나머지는 트레일링 스탑으로 운영합니다.

`.env` 설정:
```env
# 진입가 대비 이 % 수익 시 부분 익절 실행 (0 = 비활성)
PYRAMID_PARTIAL_TAKE_PCT=20.0

# 부분 익절 시 보유량의 몇 %를 매도할지 (0.1~0.9)
PYRAMID_PARTIAL_SELL_RATIO=0.5   # 50% 매도
```

동작 예시:
```
진입가 100,000원 / PARTIAL_TAKE_PCT=20.0 / PARTIAL_SELL_RATIO=0.5

현재가 120,000원 도달 시:
  → 보유량의 50%를 시장가 매도 (🟡 부분 익절 체결 텔레그램 알림)
  → 나머지 50%는 트레일링 스탑(최고가 × (1 - trail_pct%))으로 운영 계속

부분 익절은 포지션당 1회만 실행 (재시작 후에도 중복 실행 없음)
```

### 설정값 핫 리로드

재시작 없이 `.env` 변경사항을 즉시 반영합니다.

```
1. .env 파일 수정 (손절 %, 부분 익절 목표 등)
2. 텔레그램에서 /reload_settings 전송
   → ✅ 설정 재로드 완료
      • 전략 파라미터: stop=8% trail=8% ...
      • 부분 익절 목표: 0.0 → 20.0
```

재시작이 필요한 항목: API 키, 텔레그램 토큰, DB 경로, 거래 모드
