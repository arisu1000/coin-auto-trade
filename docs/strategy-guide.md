...
> **진입 봉 예외(휩쏘 방지):** 캔들 HIGH/LOW는 **봉 시작 시각이 진입 시각 이후인 봉**에서만 사용합니다. 진입 직후의 봉(특히 진입탐색용 일봉)은 진입 _이전_ 가격까지 고저가에 포함하므로, 그 고가로 최고가를 부풀리고 진입 전 저가로 트레일을 트리거하면 매수 1봉 만에 같은 가격으로 즉시 청산되는 휩쏘가 발생합니다. 진입 봉에서는 현재가(종가)만 사용합니다.
